...
export default function ActualiteEditModal({ open, onClose, actualite, onSaved }) {
  ...
  useEffect(() => {
    ...
    setDatePublication(actualite.date_publication ? String(actualite.date_publication).substring(0,10) : "");
    setFile(null);
  }, [actualite, open]);

  const previewUrl = useMemo(() => (file ? URL.createObjectURL(file) : (actualite?.image || "")), [file, actualite]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (titre !== actualite.titre) fd.append("titre", titre);
      if (contenu !== actualite.contenu) fd.append("contenu", contenu);
      if (categorie !== (actualite.categorie || "")) fd.append("categorie", categorie);
      if (datePublication !== (actualite.date_publication || "").substring(0,10)) fd.append("date_publication", datePublication || "");
      if (file) fd.append("image", file);

      const { data } = await api.put(`/actualites/${actualite.id}`, fd, { headers: { "Content-Type": "multipart/form-data" }});
      onSaved?.(data);
      onClose?.();
    } catch (err) {
      console.error("Erreur édition actualité :", err);
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="w-full max-w-3xl bg-white dark:bg-gray-900 rounded-xl shadow-xl">
        <div className="flex items-center justify-between border-b px-4 py-3">
          <h3 className="font-semibold">Modifier l’actualité</h3>
        ...
        <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-4 p-4">
          <div className="space-y-3">
            ...
            <input className="w-full border rounded px-3 py-2" value={categorie} onChange={(e)=>setCategorie(e.target.value)} />
